...
function(config, L, validate, Stapes, QMaps, sv_svc) {
  var StreetViewModule = Stapes.subclass({
    ...
    _getForwardLink: function() {
      var pov = this.streetview.getPov();
      var links = this.streetview.getLinks();    //????  comment by zoomin
      var len = links.length;
      var nearest = null;
	 ...
	 _getSosoForwardLink: function(cb) {
		 var pov = this.streetview.getPov();
		 var alpha = pov.heading/180*Math.PI;
		 var position = this.streetview.getPosition();
		  
		 var distance = config.display.move_Range;   //200 meters along heading
		 //var nextLoc = this._getLatLngByDis(distance,alpha,position);
		 var nextLoc = QMaps.geometry.spherical.computeOffset(position, distance, pov.heading);
		            
		 sv_svc.getPanoramaByLocation(
		 nextLoc,
		 distance,
		 function(data, search_latlng) {
			 if(data != null) {
	  ...
	  _getSosoBackwardLink: function(cb) {
		  var pov = this.streetview.getPov();
		  //this.setPov({ heading: (pov.heading+180)%360, pitch: pov.pitch }); //we should change heading at the same time?
